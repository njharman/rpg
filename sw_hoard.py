...
values are rounded, reordered potions and Minor Magic roll so healing more common
...
SCROLLS_PROTECTION = [
    (
        "Protection from Demons: All within a 10' radius around the reader are protected from the attacks of 1 demon "
        'per round. Duration: 40 minutes.'
    ),
    (
        "Protection from Drowning: All within a 10' radius of the reader gain the ability to breathe underwater. "
        'Duration: 1 full day.'
    ),
    'Protection from Elementals: This scroll protects against a single elemental. Duration: 40 minutes.',
    (
        "Protection from Magic: An anti-magic shell with a radius of 10' surrounds and moves with the reader; spells "
        'cannot pass in or out of the shell. Duration: 1 hour.'
    ),
    'Protection from Metal: Metal cannot harm the reader. Duration: 1 hour.',
    (
        'Protection from Poison: Poisons have no effect upon one who reads such a scroll aloud; moreover, any poisons, '
        "active or quiescent, in the scroll reader's body are instantly removed. Duration: 6 hours."
    ),
    (
        "Protection from Undead: All within a 10' radius of the reader are protected against undead, but only to a "
        'limited degree. In any given round, 2d12 undead with fewer than 4 HD, and 2d6 undead with 4-5 HD, and d6 '
        'undead with 6+ HD are foiled by the protection of the scroll. Thus, the scroll is effective against all but a '
        'vast horde of undead. Duration: 1 hour.'
    ),
    (
        "Protection from Were-creatures: All within a 10' radius around the reader are protected from lycanthropes. "
        'Duration: 1 hour.'
    ),
]
...
CURSES = [
    'Blindness (3d6 turns)',
    (
        'Causes an aversion: roll d6. The character gains a strong aversion to: (1) Swords, (2) Spiders, (3) Armor, '
        '(4) Spell casting, (5) Bathing, (6) Being underground.'
    ),
    ...
    (
        'Dimensional Vortex. The character is physically sucked up into the scroll, appearing as a new word on the '
        'page until rescued by a remove curse spell.'
    ),
    (
        'Hallucinations (3d6 turns). The exact nature of the hallucinations varies, but in general the character will '
        'either begin casting spells and/or attacking nearby people, or will remain fascinated by the colors, '
        'unwilling to move.'
    ),
    'Instant Death',
    ...
    'Paralysis (3d6 turns)',
    (
        "Paralysis: Everyone in a radius of 20' must make a saving throw or be paralyzed for 3d6 turns, with the "
        'exception of the reader of the scroll, who is unaffected.'
    ),
    (
        'Permanent diminution in size. Some of these reduce the reader to half size (50%) and the rest reduce the '
        'reader to 6 inches tall.'
    ),
    (
        'Polymorph: roll d6. The character turns into (1) a pig, (2) a mouse, (3) a flamingo, (4) a talking parrot, '
        '(5) a dog, (6) a water buffalo'
    ),
    (
        'Sleep (until curse is removed). In some cases, magical sleep can be broken by a kiss. Otherwise, the curse '
        'can only be removed by magic.'
    ),
    'Smell. The character becomes foul-smelling for 1d8 days',
    'Turned to Stone',
    (
        'Uncontrollable sneezing (3d6 turns). The reader is likely to attract wandering monsters, especially those '
...
MINOR_WEAPON_ABILITY = [
    ...
    (
        8,
        (
            'Inflicts an additional +4 points of damage against a particular opponent type such as undead, dragons, '
            'demons, giants, etc.'
        ),
    ),
]
...
def TableFactory(table):
    def func(die, table=table):
        ...
        for x, p in table:
            if roll <= x:
                return p
        raise ValueError(f'roll [{x}] is past end of table')
...
def scroll(roll):
    ...
    return [
        ...
        prot,
        f'{sphere} {pick(2, d4)}',
        f'{sphere} {pick(2, lambda: d6() + 1)}',
        f'{sphere} {pick(1, lambda: d6() + (1, 3)[sphere == "Magic-user"])}',
        f'{sphere} {pick(5, d3)}',
        curse,
...
def arms_n_armor(roll):
    ...
    return [
        f'Cursed {armor}; {curse}',
        f'+1 {missile_weapon(d20)}',
        '+1 shield',
        f'+1 {melee_weapon(d20)}',
        f'+1 {armor}',
        f'Cursed {melee_weapon(d20)}; {curse}',
        f'+2 {missile_weapon(d20)}',
        '+2 shield',
        f'+2 {melee_weapon(d20)}',
        f'+2 {armor}',
        f'+2 {melee_weapon(d20)}',
        f'+1 {melee_weapon(d20)}; {minor_weapon_ability(d8)}',
        f'+3 {missile_weapon(d20)}',
        f'+3 {melee_weapon(d20)}',
        '+3 shield',
        f'+3 {armor}',
        random.choice(
            (
                f'+1 blunt {(w := melee_weapon(d20))} that destroys undead',
                f'+1 thrown {w} that returns to hand',
                f'+1 {w}, extra attack',
                f'{w}, +1, +2 vs. particular type of foe',
                f'{w}, +1, +4 vs. particular type of foe',
                f'{w}, +2, +3 vs. particular type of foe',
                f'+4 {w}',
                f'+5 {w}',
                f'Flaming {w}',
                f'Freezing {w}',
...
def group_gems(gems):
    ...
    for value, count in sorted(collections.Counter(gems).items()):
        if count > 1:
            grouped.append(f'{count}x{value}')
        else:
...
def group_potions(potions):
    ...
    for potion, count in sorted(collections.Counter(potions).items()):
        if count > 1:
            grouped.append(f'{potion} x {count}')
        else:
            grouped.append(f'{potion}')
    return grouped
...
def generate(amount):
    ...
    if len(potions) > 1:
        print(f'Potions of {", ".join(potions[:-1])}, and {potions[-1]}')
    elif potions:
        print(f'Potion of {potions[0]}')
    if scrolls:
        scrolls.sort()
        print('\n'.join(scrolls))
    if other:
        other.sort()
        print('\n'.join(other))
    print(f'\nTotal monetary value: {gp + sum(gems)}')
    if gp:
        print(f'Coins: {gp} gp')
    if gems:
        print(f'Gems & Jewelry: {"gp, ".join(group_gems(gems))}gp')


if __name__ == '__main__':
    if len(sys.argv) != 2:
        print('Usage:')
        print(f'{sys.argv[0]} <gp value of hoard>')
    else:
