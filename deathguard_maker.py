...
class AutoTable(Table):
    ...
    def __init__(self, *table):
        ...
        super().__init__(lambda: random.randint(1, total), *new_table)
...
class FighterType:
    def __init__(self, level=1, spell_warped=False):
        ...
        if self.spell_warped:
            self.type = f"""Level {self.level} K'jore Deathguard Spellwarped Fighter (medium aberration)"""
            self.special_defenses.insert(0, f'SR {11 + self.level}')
        else:
            self.type = f"""Level {self.level} K'jore Deathguard Fighter (medium aberration)"""
    ...
    def pretty_print(self):
        ...
        print(self.type)
        print(indent(f'Hits: {" / ".join(map(str, self.hitpoints))}'))
        print(indent(f'Move: {", ".join(self.moves)}'))
        print(indent('\n'.join(self.stats)))
        print(indent(', '.join(self.saves)))
        print(indent('AC: ' + ', '.join(self.armorclasses)))
        print(indent('SD: ' + '; '.join(self.special_defenses)))
        print(indent('Attacks: {self.bab}, {self.grapple}'))
        print(stuff('\n'.join(self.attacks)))
        print(indent('Powers:'))
        print(stuff('\n'.join(self.powers)))
        print(indent('Feats:'))
    ...
    def _get_DC_result(self, table):
        ...
        args = {'dc': save + mod(self.cha), 'level': self.level}
    ...
    def _set_base_powers(self):
        ...
        self.powers = []
        self.powers.insert(0, f"""10' Darkness Despair(-1 actions) DC{10 + half_hd + cha_mod} Will""")
        if self.spell_warped:
            self.powers.insert(0, """Absorb Spell (+4 Str/Dex/Con, 10/energy, lvl*5 vitality, lvl*5' speed""")
        for _i in range(self.level // 5):
    ...
    def _set_base_magic(self):
        def bonii(bonus, item, extra=''):
            things = []
            if bonus:
                things.append(f'{bonus:+}')
            things.append(item)
            if extra:
                things.append(f'[{extra}]')
            return ' '.join(things)
        self.items = []
    ...
    def _set_base_feats(self):
        ...
        if level >= 18:
            self.feats.append(str(Feats))
        self.feats.insert(0, f'Specialization Series {self.feat_tohit:+}/{self.feat_damage:+}')

    @property
    def moves(self):
        moves = []
        moves.append(f'{self.base_move}\'')
        return moves
    ...
    def stats(self):
        ...
        for stat in ('str', 'dex', 'con', 'int', 'wis', 'cha'):
            ...
            stats.append(f'{stat.title()} {value:2} ({mod(value):+})')
        return stats
    ...
    def saves(self):
        ...
        return [f'Fort {fort:+2}', f'Ref {ref:+2}', f'Will {will:+2}']
    ...
    def attacks(self):
        ...
        for tohit in range(self._bab_value, 0, -5):
            args = {'melee_tohit': tohit + melee_bonus, 'melee_damage': melee_damage, 'range_tohit': tohit + range_bonus}
            attacks.append(weapon % args)
        return attacks

    @property
    def bab(self):
        return f'Bab {self._bab_value:+}'

    @property
    def grapple(self):
        return f'Grap {self._bab_value + mod(self.str):+}'
    ...
    def armorclasses(self):
        ...
        return [f'{ac:+2}', f'Touch {touch:+2}', f'Flat {flat:+2}']
