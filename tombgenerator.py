...
def gen_gold(roll):
    ...
    if roll <= 60:
        return f"{d(100) * 2} gp"
    if roll <= 70:
        return f"{d(6) * 1000:,} gp"
    if roll <= 80:
        return f"{d(12) * 1000:,} gp"
    if roll <= 90:
        return f"{d(20) * 1000:,} gp"
    if roll <= 99:
        return f"{random.randint(2, 40) * 1000:,} gp"
    return '100,000 gp'


def gen_gems(roll):
    if roll <= 50:
        return f"{d(6)} gems (base 100 gp)"
    if roll <= 60:
        return f"{d(6)} gems (base 500 gp)"
    if roll <= 70:
        return f"{d(8)} gems (base 500 gp)"
    if roll <= 80:
        return f"{d(12)} gems (base 500 gp)"
    if roll <= 90:
        return f"{d(12)} gems (base 1,000 gp)"
    if roll <= 99:
        return f"{d(6)} gems (base 5,000 gp)"
...
def gen_jewelry(roll):
    if roll <= 50:
        return f"{d(6)} jewelry (base 500 gp)"
    if roll <= 60:
        return f"{d(6)} jewelry (base 1,000 gp)"
    if roll <= 70:
        return f"{d(6)} jewelry (base 5,000 gp)"
    if roll <= 80:
        return f"{d(6)} jewelry (base 10,000 gp)"
    if roll <= 90:
        return f"{d(6)} jewelry (base 20,000 gp)"
    if roll <= 99:
        return f"{d(6)} jewelry (base 30,000 gp)"
...
def gen_misc_magic(roll):
    if roll <= 60:
        return 'None'
    if roll <= 70:
        return 'Greyhawk Misc. Magic Table I'
    if roll <= 80:
        return 'Greyhawk Misc. Magic Table II'
    if roll <= 90:
        return 'Greyhawk Misc. Magic Table III'
    if roll <= 99:
        return 'Greyhawk Misc. Magic Table IV'
...
def gen_tomb(roll):
    if roll <= 40:
        return '1 room/cave/mound'
    if roll <= 50:
        return 'Hall with spring trap + secret door at end'
    if roll <= 60:
        return f"{d(5) + 1} room/cave complex, many misleading doors"
    if roll <= 80:
        return f"{d(10)} rooms/caves, secret door to tomb, {d(10)} traps"
    if roll <= 90:
        return f"{d(10)} rooms, {d(20)} corridors, {d(19) + 1} traps, secret door"
    if roll <= 99:
        return f"{d(10)} rooms with traps, secret doors, and magical guard spells"
...
def gen_guardian(roll, depth=0):
    if roll <= 30:
        return 'None'
    if roll <= 50:
        return 'Magic spell (wizard lock, curse, etc.)'
    if roll <= 80:
        return f"{d(4)} Invisible Stalker(s)"
    if roll <= 99:
        return 'Monster from 6th level chart'
